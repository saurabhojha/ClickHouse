...
RestCatalog::Config RestCatalog::loadConfig()
{
    ...
    LOG_INFO(log, "Received catalog configuration settings: {}", json_str);
    ...
    LOG_INFO(log, "Parsed catalog configuration settings: {}", result.toString());
    return result;
}
...
RestCatalog::Namespaces RestCatalog::getNamespaces(const std::string & base_namespace) const
{
    ...
    try
    {
        ...
        LOG_INFO(log, "Loaded {} namespaces in base namespace {}", namespaces.size(), base_namespace);
        return namespaces;
    }
    catch (const DB::HTTPException & e)
    {
...
RestCatalog::Namespaces RestCatalog::parseNamespaces(DB::ReadBuffer & buf, const std::string & base_namespace) const
{
    ...
    LOG_INFO(log, "Received response: {}", json_str);
...
bool RestCatalog::getTableMetadataImpl(
    ...
    TableMetadata & result) const
{
    LOG_INFO(log, "Checking table {} in namespace {}", table_name, namespace_name);
    ...
    if (buf->eof())
    {
        LOG_INFO(log, "Table doesn't exist (endpoint: {})", endpoint);
        return false;
    }
    ...
    LOG_INFO(log, "Received metadata for table {}: {}", table_name, json_str);
#endif
    LOG_INFO(log, "Received metadata for table {}: {}", table_name, json_str);
    ...
    if (result.requiresLocation())
    {
        if (metadata_object->has("location"))
        {
            ...
            LOG_INFO(log, "Location for table {}: {}", table_name, location);
        }
        else
        {
    ...
    if (result.isDefaultReadableTable() && result.requiresCredentials() && object->has("config"))
    {
        ...
        switch (storage_type)
        {
            case StorageType::S3:
            {
                ...
                LOG_INFO(log, "Sko endpoint check in variable {}: in result {}", storage_endpoint, result.getEndpoint());
                break;
            }
            default:
                break;
